...
int bam_flagstat(int argc, char *argv[])
{
	...
	printf("%I64d + %I64d in total (QC-passed reads + QC-failed reads)\n", s->n_reads[0], s->n_reads[1]);
	printf("%I64d + %I64d duplicates\n", s->n_dup[0], s->n_dup[1]);
	printf("%I64d + %I64d mapped (%.2f%%:%.2f%%)\n", s->n_mapped[0], s->n_mapped[1], (float)s->n_mapped[0] / s->n_reads[0] * 100.0, (float)s->n_mapped[1] / s->n_reads[1] * 100.0);
	printf("%I64d + %I64d paired in sequencing\n", s->n_pair_all[0], s->n_pair_all[1]);
	printf("%I64d + %I64d read1\n", s->n_read1[0], s->n_read1[1]);
	printf("%I64d + %I64d read2\n", s->n_read2[0], s->n_read2[1]);
	printf("%I64d + %I64d properly paired (%.2f%%:%.2f%%)\n", s->n_pair_good[0], s->n_pair_good[1], (float)s->n_pair_good[0] / s->n_pair_all[0] * 100.0, (float)s->n_pair_good[1] / s->n_pair_all[1] * 100.0);
	printf("%I64d + %I64d with itself and mate mapped\n", s->n_pair_map[0], s->n_pair_map[1]);
	printf("%I64d + %I64d singletons (%.2f%%:%.2f%%)\n", s->n_sgltn[0], s->n_sgltn[1], (float)s->n_sgltn[0] / s->n_pair_all[0] * 100.0, (float)s->n_sgltn[1] / s->n_pair_all[1] * 100.0);
	printf("%I64d + %I64d with mate mapped to a different chr\n", s->n_diffchr[0], s->n_diffchr[1]);
	printf("%I64d + %I64d with mate mapped to a different chr (mapQ>=5)\n", s->n_diffhigh[0], s->n_diffhigh[1]);
#else
